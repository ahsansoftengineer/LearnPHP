    ...
    <div class="row">
      <div class="col-md-8">
        ...
        <div class="post-container">
          <?php
          $sql = "SELECT first_name FROM user WHERE user_id = {$author}";
          ...
          <ul class='pagination admin-pagination'>
            <?php if ($pageNo > 1) { ?>
              <li><a href="author.php?author=<?php echo ($author) ?>&page=<?php echo ($pageNo - 1) ?>">Prev</a></li>
            <?php }
            for ($page = 1; $page <= $pages; $page++) { ?>
              <li class="<?php echo $page == $pageNo ? 'active' : '' ?>">
                <a href="author.php?author=<?php echo ($author) ?>&page=<?php echo ($page) ?>">
                  <?php echo $page ?>
                </a>
              </li>
            <?php }
            if ($pageNo < $pages) { ?>
              <li><a href="author.php?author=<?php echo ($author) ?>&page=<?php echo ($pageNo + 1) ?>">Next</a></li>
            <?php } ?>
          </ul>
        </div><!-- /post-container -->
      </div>
